...
class Model:
    ...
    def dump_weights(self):
        w1, b1 = self.components[0].weights, self.components[0].bias
        w2, b2 = self.components[3].weights, self.components[3].bias
        w3, b3 = self.components[7].weights, self.components[7].bias
        w4, b4 = self.components[9].weights, self.components[9].bias
        w5, b5 = self.components[11].weights, self.components[11].bias
...
def load_data_from_csv(csv_dir, csv_files):
    print("Loading data...")
    images = []
    labels = []
    image_dirs = []
    for csv_file in csv_files:
        image_dirs.append(os.path.join(csv_dir, csv_file.split('.')[0]))

    zipped = zip(image_dirs, csv_files)

    for image_dir, csv_file in zipped:
        ...
        for files in os.listdir(image_dir):
            if files.endswith(".png"):
                if len(images) >= 500:
                    break
                image = cv2.imread(os.path.join(image_dir, files), cv2.IMREAD_GRAYSCALE)
                ...
                label = df.loc[df['filename'] == files, 'digit'].values[0]
...
if __name__ == "__main__":
    ...
    images, labels = load_data_from_csv('Dataset/', ['training-a.csv', 'training-b.csv', 'training-c.csv'])

    x_train = images[:int(0.8 * len(images))]
    y_train = labels[:int(0.8 * len(labels))]
    x_val = images[int(0.8 * len(images)):]
    y_val = labels[int(0.8 * len(labels)):]
    ...
    for lr in learning_rates:
        ...
        f1, model = train(x_train, y_train, x_val, y_val, batch_size, num_classes, lr, epochs)
        if f1 > max_f1:
            max_f1 = f1
    ...
    w1, b1, w2, b2, w3, b3, w4, b4, w5, b5 = selected_model.dump_weights()
    with open('1705051_model.pkl', 'wb') as f:
        pickle.dump((w1, b1, w2, b2, w3, b3, w4, b4, w5, b5 ), f)

    x_test, y_test = load_data_from_csv('Dataset/', ['training-d.csv'])

    print("Test data size: ", len(x_test))

    test(selected_model, x_test, y_test, num_classes)
